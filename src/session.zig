...
pub const Entry = extern struct {
    cmd_hash: u64, out_hash: u64, timestamp: u64, data_off: u32, data_len: u32, original_len: u32,
    hits: u16, flags: u16, category: u8, _pad: [7]u8 = .{0} ** 7,
};
comptime {
...
pub const Session = struct {
    ...
    pub fn header(self: *Session) *Header { return @ptrCast(@alignCast(self.map.ptr)); }
    pub fn entrySlice(self: *Session) [*]Entry { return @ptrCast(@alignCast(self.map.ptr + @sizeOf(Header))); }
    pub const lookup = ops.lookup;
    pub const invalidateCategory = ops.invalidateCategory;
    pub fn tryLock(self: *Session) bool { return lock_io.tryExclusive(self.file); }
    pub fn unlock(self: *Session) void { lock_io.unlock(self.file); }
...
test { _ = @import("session_test.zig"); }
